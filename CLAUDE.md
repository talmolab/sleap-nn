...
- Top-down (crop-centered) segmentation: model type `centered_instance_segmentation` — a lone `SegmentationHead` on a centroid crop predicting the centered instance's binary mask. `sleap-nn predict` composes a `centroid` dir + the seg dir (or the seg dir alone via a GT-centroid fallback) and emits `sio.PredictedSegmentationMask` on `frame.masks` at full-frame offsets; mask-IoU eval (`--match_method mask`). Training reads per-instance masks from `lf.masks` (linked via `mask.instance`); geometric aug is skipped in v1. See `docs/guides/topdown-segmentation.md`.
