...
- Centroid focal loss: opt-in CenterNet/CornerNet-style penalty-reduced pixelwise focal loss for the centroid confmap head, as an alternative to plain MSE — targets confmap-collapse/duplicate-detection in dense-instance scenes. Config lives on `model_config.head_configs.centroid.confmaps` (`use_sigmoid_activation` + `focal_loss_alpha`/`_beta`/`_pos_threshold`), all defaulting off (byte-identical when disabled). Val/eval loss always stays plain MSE regardless of `alpha` so checkpoints stay comparable. Centroid-only for now (not yet implemented for `single_instance`/`centered_instance`/`bottomup` — tracked in [#736](https://github.com/talmolab/sleap-nn/issues/736)); dataset-dependent results, not a universal win. See `docs/guides/centroid-focal-loss.md`.
...
- Re-ID identity/embedding persistence: predicted animal identity and appearance embeddings persist into the `.slp` via the **simplified** sleap-io `Identity`/`Embedding` data model (sleap-io #535/#536, released in `sleap-io>=0.9.2`, which `pyproject.toml` pins from PyPI). Each detection modality carries a single `identity_embedding: sio.Embedding | None` slot (bare value object wrapping just a `vector` — no space-name/normalized/source metadata) plus `identity`/`identity_score`. (1) **Embeddings:** `sleap-nn predict` on an `embedding` model takes `--save_embeddings {none,slp}` (default `none`); `slp` sets `det.identity_embedding = sio.Embedding(vector)` on EVERY source detection, tracked or not (object-exact via `EmbeddingDataset.mask_idx_list[item_id]["mask_obj"]`; vectors already on the input are cleared first so a file never mixes two models' vectors) and writes the `.slp` **with `save_embedding_vectors=True`** (the #536 default flipped to `False`, so every persist call passes the flag explicitly; identity/Track links persist regardless). The shared embed kernel is `inference/embedding.py::embed_labels`; `predict_embeddings_to_slp` wraps it. Both pose `Instance` and mask `SegmentationMask` embeddings persist (mask-modality `owner_type=3`). The embedding pathway stores **vectors only** — it does NOT fabricate `sio.Identity` from track/class names (existing identities pass through). (2) **Identity:** a multi_class model's class output is interpreted per its head config `class_output` ∈ `{"track"` (default), `"identity"}` (validated at config load). `"track"` (default) emits only the per-video `sio.Track`. `"identity"` ADDITIVELY assigns a canonical global `sio.Identity(name=<class name>)` + `identity_score` (the class probability) alongside the Track — for classes that enumerate **unique individuals**. The simplified `Identity` matches by **name** (#535 dropped `uuid`), so the class NAME is the canonical cross-file/cross-retrain key: no per-class uuid is minted or carried anywhere (the old train-time freeze and the `class_uuids` config field are both deleted; `_multiclass_identities` builds `sio.Identity(name=...)`, dedup by name). Any other `class_output` raises `NotImplementedError` — sleap-io does export `sio.Category`, but mapping classes onto it is not implemented. `Identity` (global) + `identity_score` mirror `Track`/`tracking_score`; GT-vs-predicted is the host class (`PredictedInstance`) + `identity_score is not None`.
- Embedding (appearance / re-ID) tracking: the core `Tracker` gains a `features="embeddings"` mode — an alternative to pose→`oks`, it reads each detection's appearance vector from the single `identity_embedding` slot (`get_embedding` → `inst.identity_embedding.vector`, works on pose `PredictedInstance` *and* mask `PredictedSegmentationMask`) and scores candidates by `cosine_sim` (hardened: `None`/zero-norm/shape-mismatch → `NaN` → no match). Registered in `Tracker._feature_methods`; `apply_tracking` (`inference/tracking.py`) auto-pairs `embeddings`→`cosine_sim` (scored by the vectorized `_appearance_scores` matmul, not a per-pair loop), defaults the candidate maker to `local_queues` when `--candidates_method` is unset (post-occlusion re-ID), skips the single-node/mask auto-resolution that would clobber it, routes to the carrier holding MORE vectors (`embedding_carriers(labels).dominant` — the count-based `count_carriers`/`CarrierCounts` resolver, reusable for other "which carrier?" questions), and rejects motion models (`--use_flow`/`--use_kalman`) + (mask-carrier) pose cull/clean/connect. The same resolver guards `--appearance_weight`: the blend reads the TRACKED carrier's own vectors, so vectors only on the other carrier raise (on a pose+mask file the embedder puts them on the carrier the model was TRAINED on — `head_configs.embedding.embedding.detection_mode`, recorded at training; a pre-field config counts as mask-trained under `burn_in`, else count-based with ties→mask — so a mask-trained model's vectors sit on the masks: blend there with `--features masks`, which tracks the mask carrier); a ≥10% vector-less tracked carrier warns. `cosine_sim` is rejected as the geometric side of a blend. **`apply_tracking` works IN PLACE** (documented contract, not a copy): it writes tracks onto the input's detections, replaces only each frame's tracked carrier, reorders frames to `(video, frame_idx)`, rebuilds `labels.tracks`, and returns the input — so masks/ROIs/centroids/suggestions/sessions/provenance pass through (callers wanting the input intact pass `labels.copy()`). One fresh `Tracker` per video (track names continue across videos via `Tracker.track_name_offset`; post-cleanup is per video too). In mask-carrier mode each pose linked to exactly one tracked mask (`mask.instance`) inherits its track. Three CLI workflows: **(WF1)** retrack a saved-embeddings `.slp` — `sleap-nn predict -i embedded.slp -t --features embeddings` (track-only path: no model load and no GPU needed, but NOT torch-free -- `import sleap_nn` imports torch via `sleap_nn/__init__` -> `evaluation.py`). **(WF2)** embed + track from a detections `.slp` — `sleap-nn predict -m <embedding_model> -i detections.slp -t`: `predict_embeddings_to_slp(tracker_config=…)` embeds EVERY detection except empty masks (`EmbeddingDataset(include_untracked=True)` enumerates untracked + predicted pose/mask, `group_id=0` placeholder), attaches vectors, runs `apply_tracking`, writes a tracked `.slp`; `--save_embeddings {none,slp}` controls whether vectors persist in it (`none`=tracks only, stripped via `_strip_embeddings` + `save_embedding_vectors=False`). **(WF3)** fused detect+embed+track from a video — `sleap-nn predict -m <centroid> [-m <centered_instance>] -m <embedding> -i video.mp4 -t`: `cli._run_embeddings` splits detection vs embedding dirs (`_is_embedding_model`), runs the detection stack and hands the `sio.Labels` it returns to WF2 in memory (`predict_embeddings_to_slp(labels=...)`, no temp `.slp`). Emits `sio.Track` only (no global `Identity`). Guide: `docs/guides/embedding-tracking.md`.
