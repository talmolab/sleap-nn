...
class VideoReader(Thread):
    ...
    queue as a tuple in the format (image, frame index, video index, (height, width))
        ...
        try:
            for idx in range(self.start_idx, self.end_idx):
                ...
                self.frame_buffer.put(
                    (
                        torch.from_numpy(img),  # img
                        torch.tensor(idx, dtype=torch.int32),  # frame idx
                        torch.tensor(0, dtype=torch.int32),  # video idx
                        torch.Tensor(img.shape[-2:]),  # orig shape
                    )
                )
...
class LabelsReader(Thread):
    ...
    buffer queue as a tuple in the format (image, frame index, video index, (height, width))
    which are then batched and consumed during the inference process.
    ...
    def from_filename(
        ...
        labels = sio.load_slp(filename)
        frame_buffer = Queue(maxsize=queue_maxsize)
        return cls(labels, frame_buffer)
    ...
    def run(self):
        ...
        try:
            ...
            self.frame_buffer.put((None, None, None, None))
