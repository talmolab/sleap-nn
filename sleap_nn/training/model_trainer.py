    ...
    def __init__(
        ...
        if (
            ...
            if self.cache_img == "disk":
                ...
                self.train_cache_img_paths = {}
                self.val_cache_img_paths = {}

                for d_num, d_name in self.config.dataset_mapper.items():
                    self.train_cache_img_paths[d_num] = (
                        Path(self.cache_img_path) / f"{d_name}" / "train_imgs"
                    )
                    self.val_cache_img_paths[d_num] = (
                        Path(self.cache_img_path) / f"{d_name}" / "val_imgs"
                    )

                    if self.use_existing_imgs[d_num]:
                        if not (
                            self.train_cache_img_paths[d_num].exists()
                            and self.train_cache_img_paths[d_num].is_dir()
                            and any(self.train_cache_img_paths[d_num].glob("*.jpg"))
                        ):
                            message = f"There are no images in the path: {self.train_cache_img_paths[d_num]}"
                            logger.error(message)
                            raise Exception(message)
            ...
            raise Exception(f"{self.data_pipeline_fw} is not supported!")
        ...
        # set skeletons, compute preprocessing params from labels file
        for d_num, d_name in self.config.dataset_mapper.items():
            self.train_labels[d_num] = sio.load_slp(
                self.config.data_config.train_labels_path[d_num]
            )
            self.val_labels[d_num] = sio.load_slp(
                self.config.data_config.val_labels_path[d_num]
            )

            if self.config.data_config.preprocessing.scale[d_num] is None:
                self.config.data_config.preprocessing.scale[d_num] = 1.0

            self.skeletons_dict[d_num] = self.train_labels[d_num].skeletons
            ...
            self.edge_inds[d_num] = self.train_labels[d_num].skeletons[0].edge_inds
    ...
    def _create_data_loaders_torch_dataset(self, d_num):
        ...
        if self.data_pipeline_fw == "torch_dataset_cache_img_memory":
            train_cache_memory = check_memory(
                ...
                crop_size=self.crop_hws[d_num] if self.crop_hw != -1 else None,
            )
            val_cache_memory = check_memory(
                self.val_labels[d_num],
                max_hw=(self.max_heights[d_num], self.max_widths[d_num]),
                model_type=self.model_type,
                input_scaling=self.config.data_config.preprocessing.scale[d_num],
                crop_size=self.crop_hws[d_num] if self.crop_hw != -1 else None,
            ...
            if total_cache_memory > available_memory:
                raise Exception(
                    f"Insufficient memory for in-memory caching. Use disk caching instead."
                )
        ...
        # If using caching, close the videos to prevent `h5py objects can't be pickled error` when num_workers > 0.
        if "cache_img" in self.data_pipeline_fw:
            for video in self.train_labels[d_num].videos:
                if video.is_open:
                    video.close()

            for video in self.val_labels[d_num].videos:
                if video.is_open:
                    video.close()

        if self.config.trainer_config.trainer_devices > 1:
            # add sampler if using more than 1 gpu
    ...
    def train(self):
        ...
        try:
            ...
            if (
                ...
                for d_num, d_name in self.config.dataset_mapper.items():
                    if (self.train_cache_img_path[d_num]).exists():
                        shutil.rmtree(
                            (self.train_cache_img_path[d_num]).as_posix(),
                            ignore_errors=True,
                        )

                    if (self.val_cache_img_path[d_num]).exists():
                        shutil.rmtree(
                            (self.val_cache_img_path[d_num]).as_posix(),
                            ignore_errors=True,
                        )
