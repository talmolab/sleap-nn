...
def get_head_configs(head_cfg: Union[str, Dict[str, Any]]):
    ...
    if isinstance(head_cfg, str):
        if head_cfg == "centered_instance":
            ...
            raise ValueError(
                f"{head_cfg} is not a valid head type. Please choose one of ['bottomup', 'centered_instance', 'centroid', 'single_instance', 'multi_class_bottomup', 'multi_class_topdown', 'bottomup_segmentation', 'centered_instance_segmentation', 'semantic_segmentation', 'embedding']"
            )
...
def get_trainer_config(
    ...
    Args:
        ...
        ckpt_save_last: When True, also writes a last.ckpt at the end of every epoch
            ...
            whether or not it improved the monitored metric. Default: False.
