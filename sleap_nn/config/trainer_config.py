...
class TrainerConfig:
    ...
    Attributes:
        ...
        trainer_accelerator: (str) One of the ("cpu", "gpu", "mps", "xpu", "auto"). "auto" recognises the machine the model is running on and chooses the appropriate accelerator for the Trainer to be connected to. `"xpu"` targets Intel GPUs (Arc / Battlemage) via `torch.xpu`; sleap-nn registers a Lightning `XPUAccelerator` on import. *Default*: `"auto"`.
