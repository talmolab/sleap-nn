...
def test_bottomup_inference_model(minimal_instance_bottomup_ckpt):
    ...
    inference_layer = BottomUpInferenceModel(
        ...
        paf_scorer=PAFScorer.from_config(
            config=OmegaConf.create(
                {
                    "confmaps": train_config.model_config.head_configs[0].head_config,
                    "pafs": train_config.model_config.head_configs[1].head_config,
                }
            )
        ),
        peak_threshold=0.03,
    ...
    assert tuple(output["pred_instance_peaks"][0].shape) == (2, 2, 2)
    assert tuple(output["pred_peak_values"][0].shape) == (2, 2)
    assert tuple(output["instance_scores"][0].shape) == (2,)
