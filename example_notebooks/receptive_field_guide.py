...
@app.cell(hide_code=True)
def _(mo):
    scale = mo.ui.slider(0, 1.0, step=0.1, value=1.0, label="Choose an input scale value: ")
    max_stride = mo.ui.dropdown(options=[8, 16, 32, 64, 128], value=16, label="Choose a max_stride value: ")
...
def _(np, plt):
    ...
    def compute_receptive_field(down_blocks: int, convs_per_block: int = 2, kernel_size: int = 3) -> int:
        """
    ...
    def plot_receptive_field(
        ...
        ax.set_title(f'Receptive Field: {rf_size} pixels, Max stride: {max_stride}, Scale: {scale}', fontsize=14, fontweight='bold')
    ...
    def plot_receptive_field_comparison(
        ...
        ax.set_title(f'Receptive Field Comparison (scale: {scale})', fontsize=14, fontweight='bold')
